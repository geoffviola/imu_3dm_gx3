...
{
  static const uint8_t DEFAULT_SEND_MODE = 0XCCU;
  ...
  std::string send_mode_raw;
  uint32_t send_mode;
  const bool set_mode = n.getParam("send_mode", send_mode_raw);
  if (set_mode)
    {
      std::stringstream ss;
      ss << std::hex << send_mode_raw;
      ss >> send_mode;
      ROS_INFO("User mode set to 0x%x", send_mode);
    }
  else
    {
      send_mode = static_cast<uint32_t>(DEFAULT_SEND_MODE);
      ROS_INFO("Default mode set to 0x%x", send_mode);
    }
  ...
  else if (0xD3 == send_mode)
    {
      data_length = 43;
    }
  else
    {
      ROS_ERROR("This driver does not support mode \"0x%x\"", send_mode);
      return EXIT_FAILURE;
    }
